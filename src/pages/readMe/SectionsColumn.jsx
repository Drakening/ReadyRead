...
const SectionsColumn = () => {
  const { sections, addSection, removeSection, updateSections, darkMode } = useReadme()
  ...
  const handleRemoveSection = (index) => {
    ...
    const insertIndex = newAvailableSections.findIndex((s) => {
      const currentOriginalIndex = originalSections.findIndex((os) => os.name === s.name)
      return currentOriginalIndex > originalIndex
    })

    if (insertIndex === -1) {
      newAvailableSections.push(sectionToRemove)
    } else {
    ...
    <div className={`${styles.column} ${darkMode ? styles.darkMode : ""}`}>
      ...
      <DragDropContext onDragEnd={handleOnDragEnd}>
        <Droppable droppableId="sections">
          {(provided) => (
            <ul className={styles.sectionList} {...provided.droppableProps} ref={provided.innerRef}>
